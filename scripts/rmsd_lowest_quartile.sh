...
sd_rmsd_tmp_file="sd_rmsd_values.dat"

rm $rmsd_tmp_file $fxray_tmp_file $d05_tmp_file $sd_rmsd_tmp_file
...
do
    ...
    sd_rmsd_low_quartile=$(awk '{print $5}' $filename | head -n $low_quartile | awk -v mean=$avg_rmsd_low_quartile '
    {
    ...
    echo $sd_rmsd_low_quartile >> $sd_rmsd_tmp_file
done
...
str4=$(awk '{n++;sum+=$1} END {printf "%10.8f\n", n?sum/n:0}' $sd_rmsd_tmp_file)
